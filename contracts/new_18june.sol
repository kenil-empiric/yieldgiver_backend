...
contract YieldGivers_18jun {
    ...
    uint256 public maxInvestmentLimit = 100000 * 10**6;
    ...
    event MaxInvestmentLimitChanged(uint newLimit);
    ...
    modifier hasPassedCoolDown() {
        require(block.timestamp >= userMap[msg.sender].lastWithdrawalTime + coolDownPeriod, "Cool-down period has not passed");
        _;
    }

    function setCoolDownPeriod(uint _coolDownPeriod) public onlyOwner {
    ...
    function setMaxInvestmentLimit(uint _maxInvestmentLimit) public onlyOwner {
        maxInvestmentLimit = _maxInvestmentLimit;
        emit MaxInvestmentLimitChanged(_maxInvestmentLimit);
    }
    ...
    function stake(
        ...
        require(userMap[msg.sender].investment + Amount <= maxInvestmentLimit, "Investment exceeds the maximum limit");
    ...
    function withdraw(uint256 amount) public hasPassedCoolDown{
        amount = updateReward(amount);
        console.log("amount in withdraw -----------",amount);
        userMap[msg.sender].lastWithdrawalTime = block.timestamp;
    ...
    function withdrawPrincipal(uint256 investmentIndex) public {
        ...
        require(
            ...
            "Please wait for the Initial Lockup Period to end before you withdraw your investment. Thanks"
        );
    ...
    function addInvestment(
        ...
        console.log("rate addInvestment--------", rate,totalReward);
        ...
        if (period > 0) {
            ...
            // uint256 Totalincome= reIncome.add(totalReward);
