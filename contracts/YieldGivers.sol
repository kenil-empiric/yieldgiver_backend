...
contract YieldGivers {
    ...
    constructor(address payable dev_, address payable ad_, address payable team_, uint256 startTime_, uint256 rankTime_) {
    ...
    function getPeriodAndRate(uint typeNum, uint income) public view returns(uint256 period, uint rate, uint totalReward){
        if(typeNum==1){
            ...
            console.log("1 totalReward  inside getperiodrate--------",totalReward);
        } else if(typeNum==2){
            ...
            totalReward = calcRewardCompound(income, rate, period);
            console.log("2 totalReward  inside getperiodrate--------",totalReward);
        } else if(typeNum==3){
            ...
            console.log("3 totalReward  inside getperiodrate--------",totalReward);
        } else if(typeNum==4){
            ...
            totalReward = calcReward(income, rate, period);
            console.log("4 totalReward  inside getperiodrate--------",totalReward);
        }
    }
    ...
    function stake(address referrer, uint typeNum) public payable {
        ...
        require(income >= minInvestAmount * 10 ** (unit.sub(2)), "Please invest more than the minimum amount");
        require(income <= maxInvestAmount * 10 ** unit, "Please invest less than the maximum amount");
    ...
    function updateReward(uint amount) private returns(uint){
        ...
        console.log("total reward in update reward-----",user.totalReward);
        console.log("total withdraw in update reward-----",user.totalWithdraw);
        console.log("checkpoint in update reard-----",user.checkpoint);
        console.log("pool inside updatereward--------",pool);
    ...
    function withdraw(uint amount) public {
        ...
        if(insuranceTime > 0 && insuranceTime < block.timestamp){
            ...
            msg.sender.transfer(amount.mul(80).div(100));
    ...
    function withdrawPrincipal(uint investmentIndex) public {
        User storage user = userMap[msg.sender];
        require(investmentIndex < user.investments.length, "Invalid investment index");
        Investment storage investment = user.investments[investmentIndex];
        require(block.timestamp >= investment.finish, "Cannot withdraw principal before the investment period ends");
        ...
        msg.sender.transfer(principalAmount);
            ...
            if(investment.typeNum > 2) {
                ...
                if(start < end){
                    amount = amount.add(end.sub(start).mul(rate));
                     console.log("amount in getamount if condition-----",amount);
                }
            }
        }
    }
    ...
    function addInvestment(uint typeNum, uint income, bool isReStake) private{
        ...
        if (period > 0) {
            address(uint160(dev)).transfer(income.mul(70).div(1000));
            address(uint160(ad)).transfer(income.mul(70).div(1000));
